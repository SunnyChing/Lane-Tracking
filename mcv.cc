...
void SAVE_MAT_IMAGE(const CvMat *ppmat,string f)
{
  float a;
CvMat* mat = cvCreateMat(ppmat->rows, ppmat->cols, INT_MAT_TYPE);
CvMat* pmat = cvCreateMat(ppmat->rows, ppmat->cols, FLOAT_MAT_TYPE);
cvConvertScale(ppmat, pmat, 1);
  for(int i=0; i<pmat->rows; i++){
    for(int j=0; j<pmat->cols; j++){
     a =  CV_MAT_ELEM(*ppmat, float, i, j);

    if(a>0.0){
      // CV_MAT_ELEM(*mat, int, i, j)=1;
       cout << "\n"<<a;
    }

    }
  }
    cvSaveImage(f.c_str(),mat); 
}
...
CvMat* mcvCvPoint2D32f2Mat(const CvPoint2D32f vec[4])
{
  CvMat *mat = 0;
    //create the matrix
    mat = cvCreateMat(4, 2, CV_32FC1);
    //loop and get values
    for (int i=0; i<4; i++){
