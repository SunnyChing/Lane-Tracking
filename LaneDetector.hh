...
typedef struct Spline
{
  ...
  int label =0;
  int noUpdate =0;
  // box position for labeling
  int boxlabel;
  float cur;
} Spline;

typedef struct CvParticle2D32f
{
  CvPoint2D32f p[4];// #n { (x0,y0), (x1,y1), (x2,y2),(x3,y3)}
...
typedef struct Particle 
{
    ...
    CvPoint2D32f points[4]; //(x0,y0), (x1,y1), (x2,y2),(x3,y3)
    CvPoint2D32f points_w[4]; //point on world frame
    int num;
    vector<CvParticle2D32f> ps;
    int label;
    //int update_t;
    //cv::Mat pp;
...
typedef struct CenterLineState_
{
  ///Splines detected in IPM image
  Spline ipmSplines;
  ...
  // pitch
  float pitch;
  //initial height of camera and the lane width 
  float hc[2];
 ...
 * \param cameraInfo the camera parameters
 * \param stopLineConf parameters for stop line detection
 * \param lane_state: a particle filter is use to track the lane.
...
void mcvGetLanes(const CvMat *inImage, const CvMat* clrImage,
                 ...
                 vector<Spline> *splines, vector<float> *splineScores,IPMInfo
                 *ipmInfo,CameraInfo *cameraInfo, LaneDetectorConf *stopLineConf,
                  CvMat *dbipm, LineState* state = NULL);
...
void mcvGetRansacLines(const CvMat *im, vector<Line> &lines,
 ...
 /** This function giove label to  lines
 *
 * \param im the input image
 * \param lines vector of input lines
 * \param lineScores the scores of input lines
 ...
 * \param lineType the type of line to work on (LINE_HORIZONTAL or
 *    LINE_VERTICAL)
 * \param state from last frame
 * \param ipmInfo ipm infomation
 */
void mcvGetLinesLabel(const CvMat *im, vector<Line> &lines,
                       vector<float> &lineScores, LaneDetectorConf *lineConf,
                       LineType lineType,LineState *state,  IPMInfo *ipmInfo);
void PointWorld2ImIPM(CvPoint2D32f &point, CvPoint2D32f &new_point, IPMInfo *ipmInfo);
...
void mcvFitRansacSpline_(const CvMat *image, int numSamples, int numIterations,
                        ...
                        float splineScoreLengthRatio,
                        float splineScoreAngleRatio, float splineScoreStep,
                        vector<Spline> &splines,vector<float> &scores,vector<Spline> *prevSplines = NULL);
...
CvMat*  mcvExtendPoints_lsTangent(const CvMat *im, const CvMat *inPoints,
                        ...
                        int linePixelsTangent, int linePixelsNormal,
                        float contThreshold, int deviationThreshold,
                        CvRect bbox,Spline spline, bool smoothPeaks=true);
...
void mcvGetSplinesLabel(vector<Spline> &bestSplines,vector<float> &bestSplineScores,vector<Spline> &splines,vector<float> &splineScores, LineType type,
