                    ...
                    FLOAT sigmay, LineType lineType)
{
  ...
  {
    //this is for 5-pixels wide
    FLOAT_MAT_ELEM_TYPE derivp[] =
     {1.000000e-11, 8.800000e-10, 3.531000e-08, 8.536000e-07, 1.383415e-05,
      1.581862e-04, 1.306992e-03, 7.852691e-03, 3.402475e-02, 1.038205e-01,
       2.137474e-01, 2.781496e-01, 2.137474e-01, 1.038205e-01, 3.402475e-02,
        7.852691e-03, 1.306992e-03, 1.581862e-04, 1.383415e-05, 8.536000e-07,
         3.531000e-08, 8.800000e-10, 1.000000e-11};
       /* {1.000000e-06, 4.800000e-05, 9.660000e-04, 1.048000e-02, 6.529500e-02,
         2.278080e-01, 3.908040e-01, 2.278080e-01, 6.529500e-02, 1.048000e-02, 
         9.660000e-04, 4.800000e-05, 1.000000e-06};*/
    //Sunny change kernel 
    /*{1.000000e-16, 1.280000e-14, 7.696000e-13, 2.886400e-11, 7.562360e-10,
     ...
     1.042202e-03, 4.915650e-03, 1.829620e-02, 5.297748e-02, 1.169560e-01, 
     1.918578e-01, 2.275044e-01, 1.918578e-01, 1.169560e-01, 5.297748e-02, 
     1.829620e-02, 4.915650e-03, 1.042202e-03, 1.759328e-04, 2.374101e-05, 
     2.558828e-06, 2.189405e-07, 1.468714e-08, 7.562360e-10, 2.886400e-11, 
     7.696000e-13, 1.280000e-14, 1.000000e-16};*/
    int derivLen = 23; //23; 13; 33;
    FLOAT_MAT_ELEM_TYPE smoothp[] = {-1.000000e-03, -2.200000e-02, -1.480000e-01, -1.940000e-01, 7.300000e-01,
                                 -1.940000e-01, -1.480000e-01, -2.200000e-02, -1.000000e-03};
...
#warning "still check subtracting mean from image"
  ...
  //if (DEBUG_LINES)
  //{ //#def DEBUG_GET_STOP_LINES
    //SHOW_MAT(outImage, "Kernel:");
    //cout << "\n" << mean.val[3];
  //} //#endif
  //do the filtering
  cvFilter2D(outImage, outImage, &fy);
  //cvFilter2D(outImage, outImage, &fx); //inImage outImage  // Sunny Don't know why
...
#define LOCAL_MAX_IGNORE (int(MAX_IGNORE / 4))
  ...
  if (localMaxima)
  {
    ...
    for (int i = 1 + LOCAL_MAX_IGNORE; i < sumLines.rows - 1 - LOCAL_MAX_IGNORE; i++)
    {
      ...
      if ((val > CV_MAT_ELEM(sumLines, FLOAT_MAT_ELEM_TYPE, i - 1, 0)) && (val > CV_MAT_ELEM(sumLines, FLOAT_MAT_ELEM_TYPE, i + 1, 0))
          //		&& (i != maxLoc)
  ...
  { //#ifdef DEBUG_GET_STOP_LINES
    ...
    {
    case LINE_HORIZONTAL:
      sprintf(str, "%s", "4.Horizontal Lines");
      break;
    case LINE_VERTICAL:
      sprintf(str, "%s", "4.Vertical Lines");
      printf("4. Verticle Lines");
      break;
    }
    SHOW_IMAGE(im, str, 10);
    cin.get();
...
void mcvGetLanes(const CvMat *inImage, const CvMat *clrImage,
                 ...
                 IPMInfo *ipmInfo,CameraInfo *cameraInfo, LaneDetectorConf *stopLineConf,
                 CvMat *dbipm, LineState *state)
{
  //input size
  CvSize inSize = cvSize(inImage->width, inImage->height);
  ...
  CvSize ipmSize = cvSize((int)stopLineConf->ipmWidth,
                          (int)stopLineConf->ipmHeight);
  CvMat *ipm, *ipm_adjust;
  ipm = cvCreateMat(ipmSize.height, ipmSize.width, inImage->type);
  ///ipm_adjust = cvCreateMat(ipmSize.height, ipmSize.width, inImage->type);
  ...
  list<CvPoint> outPixels;
  list<CvPoint>::iterator outPixelsi;
  //if (cameraInfo->pitch!=3.2){
    //mcvGetIPM(image, ipm_adjust, &ipmInfo, cameraInfo, &outPixels);
    //mcvUpdateCameraInfo (cameraInfo, 3.2);}
  mcvGetIPM(image, ipm, ipmInfo, cameraInfo, &outPixels);
  //ipm = cvCloneMat(image); // Sunny for global image
...
#warning "Check: Smoothing IPM image"
  ...
  cvCopy(ipm, dbIpmImage);
  //SHOW_IMAGE(dbIpmImage, "IPM image", 10);
  //SHOW_IMAGE(ipm_adjust, "IPM image adjust pitch", 10);
  if (DEBUG_LINES)
  { //#ifdef DEBUG_GET_STOP_LINES
  ...
  //              stopLineConf->kernelHeight, sigmax, sigmay,
  //              LINE_VERTICAL);
  // do the dilation -Sunny
  /* cv::Mat dst = cv::Mat(ipm,true);
...
#warning "Check this clearing of IPM image for 2 lanes"
  ...
  if (DEBUG_LINES)
  {
    ...
    printf("2. Lane unthresholded filtered");
    cin.get();
  }
...
#warning "clean negative parts in filtered image"
  ...
  for (int i = 0; i < stopLineConf->numStrips; i++) //lines
  {
    ...
    { //#ifdef DEBUG_GET_STOP_LINES
      ...
      char str[256];
      sprintf(str, "Lanes thresholded IPM");
      //thresholded ipm
      SHOW_IMAGE(dbIpmImageThresholded, str, 10);
      printf("3. Lanes  thresholded IPM");
      cvReleaseMat(&dbIpmImageThresholded);
      cin.get();
    }
    ...
    {
      lines->push_back(subimageLines[k]);
    ...
    { //#ifdef DEBUG_GET_STOP_LINES
      for (unsigned int k = 0; k < lines->size(); k++)
        dbIpmStopLines.push_back((*lines)[k]);
  ...
  if (stopLineConf->ransacSpline)
    for (int i = 0; i < (int)dbIpmSplines.size(); i++)
    {
      ...
      //sprintf(str, "%d", dbIpmSplines[i].label);
...
//Sunny new with projct back to origin image
void mcvPostprocessLines_(const CvMat *image, const CvMat *clrImage,
                         ...
                         IPMInfo &ipmInfo, CameraInfo &cameraInfo)
{
  ...
  if (lineConf->ransacSpline)
  {
    ...
    {
      //get spline status
      int splineStatus = lineConf->checkIPMSplines ? mcvCheckSpline(splines[i],
      ...
      //check it
      if (!(((splineStatus & CurvedSpline) && (splineStatus & CurvedSplineTheta)) || splineStatus & HorizontalSpline || splineStatus & DiffDir))
      {
        ...
        CvMat *p = mcvExtendPoints_lsTangent(rawipm, points,
                                   ...
                                   cvRect(0, lineConf->extendIPMRectTop,
                                          rawipm->width - 1,
                                          lineConf->extendIPMRectBottom - lineConf->extendIPMRectTop),splines[i],
                                   false);
...
#warning "Check this later: extension in IPM. Check threshold value"
    ...
    state->ipmSplines.clear();   //Sunny
    state->ipmSplines = splines; //post process on IPM image
                         ...
                         IPMInfo &ipmInfo, CameraInfo &cameraInfo)
{
  ...
  if (lineConf->ransacSpline)
  {
    ...
    for (int i = 0; i < (int)splines.size(); i++)
    {
      ...
      //check it
      if (!(((splineStatus & CurvedSpline) && (splineStatus & CurvedSplineTheta)) || splineStatus & HorizontalSpline))
      {
        //better localize points
        CvMat *points = mcvEvalBezierSpline(splines[i], .1); //.05
        //mcvLocalizePoints(ipm, points, points); //inImage
...
#warning "Check this later: extension in IPM. Check threshold value"
    ...
    {
      //check the spline
      int splineStatus = lineConf->checkSplines ? mcvCheckSpline(splines[i],
      ...
      if (!(splineStatus & (CurvedSpline | CurvedSplineTheta)))
      {
        ...
        LineColor clr = lineConf->checkColor ? mcvGetPointsColor(clrImage, clrPoints,
                 ...
                 LaneDetectorConf *lineConf, LineState *state, IPMInfo *ipmInfo)
{
  ...
  mcvGetSplinesBoundingBoxes(splines, lineType,
                             cvSize(image->width, image->height),
                             state->ipmBoxes);
 // mcvGetSplinesLabel( bestSplines,bestSplineScores,splines, splineScores, lineType,
 //                  cvSize(image->width, image->height),
 //                   state->ipmSplines);
  
  // Sunny - replace by mcvGetSplinesBoundingBoxeslabel
  /*mcvGetSplinesBoundingBoxesLabel(splines, lineType,
                               cvSize(image->width, image->height),
                               state->ipmBoxes);*/
}
                   ...
                   float thetaThreshold)
{
  ...
  float thetaDir = meanTheta*theta;
  ...
  if (thetaDir < 0) //(fabs(meanTheta)>thetaThreshold)
    check |=DiffDir;
  if (DEBUG_LINES)
  { //#ifdef DEBUG_GET_STOP_LINES
    ...
    fprintf(stderr, "%s: curveness=%f, length=%f, thetaDiff=%f, meanTheta=%f, theta=%f\n",
            check & (ShortSpline | CurvedSpline | HorizontalSpline|DiffDir)
                ? "YES"
                : "NO ",
            curveness, length, thetaDiff, meanTheta, theta);
    fprintf(stderr, "\t%s\t%s\t%s\t%s\t%s\n",
            ...
            check & DiffDir ? "diffdir" : "not diffdir");
                                    ...
                                    Line &outLine)
{
  ...
  //points of intersection
  FLOAT_POINT2D pts[4] = {{xup, rect.startPoint.y}, {xdown, rect.endPoint.y}, {rect.startPoint.x, yleft}, {rect.endPoint.x, yright}};
...
{
  return (point.x >= rect.startPoint.x && point.x <= rect.endPoint.x && point.y >= rect.startPoint.y && point.y <= rect.endPoint.y) ? true : false;
}
...
{
  return (point.x >= rect.x && point.x <= (rect.x + rect.width) && point.y >= rect.y && point.y <= (rect.y + rect.height)) ? true : false;
}
...
void mcvDrawPoints(CvMat *image, CvPoint2D32f p, CvScalar color, int width)
{
  //put the control points with circles
    cvCircle(image, cvPointFrom32f(p), width, color, -1);
}
...
CvMat *mcvGetLinePixels(const Line &line)
{
  //get two end points
  CvPoint start;
  start.x = int(line.startPoint.x);
  start.y = int(line.startPoint.y);
  CvPoint end;
  end.x = int(line.endPoint.x);
  end.y = int(line.endPoint.y);
    ...
    for (int i = 0; i < pixels->height; i++)
    {
      CV_MAT_ELEM(*pix, FLOAT_MAT_ELEM_TYPE, 0, i) =
          CV_MAT_ELEM(*im, FLOAT_MAT_ELEM_TYPE,
                      changey ? min(max(CV_MAT_ELEM(*pixels, int, i, 1) +
                                            changes[c],
                                        0),
                                    im->height - 1)
                              : CV_MAT_ELEM(*pixels, int, i, 1),
                      changey ? CV_MAT_ELEM(*pixels, int, i, 0) : min(max(CV_MAT_ELEM(*pixels, int, i, 0) + changes[c], 0), im->width - 1));
                   ...
                   float groupThreshold, CvSize bbox)
{
  ...
  for (int i = 0; i < (int)rs.size(); i++)
  {
    ...
    vector<Line>::iterator iline, ilinex;
    for (ilinex = lines.begin(), iscore = lineScores.begin(), iline = lines.begin();
         ilinex != lines.end() && iscore != lineScores.end() && line.startPoint.x <= (*ilinex).startPoint.x; iscore++, ilinex++, iline++)
      ;
    line.rtheta[0]=rs[i];
    line.rtheta[1]=thetas[i];
    lineScores.insert(iscore, newScores[i]);
    lines.insert(iline, line);
    /*for (ilinex=lines.begin(),iscore=lineScores.begin();ilinex!=lines.end()&& line.startPoint.x<=(*ilinex).startPoint.x;ilinex++,iscore++){
      lineScores.insert(iscore, newScores[i]);
      lines.insert(ilinex, line);
    }*/
  }
  //clear
  newScores.clear();
...

{
  ...
  { //#ifdef DEBUG_GET_STOP_LINES
    ...
    printf("4. spline before grouping");
    cin.get();
    //clear
    cvReleaseMat(&im);
  ...
  { //#ifdef DEBUG_GET_STOP_LINES
    ...
    printf("5. splines after grouping");
    cin.get();
    //clear
    cvReleaseMat(&im);
...
//Sunny
/** This function groups nearby splines for measurement
 *
 * \param splines vector of splines
 * \param lineScores scores of input lines
 */
    ...
    for (i = boxes.begin(); i != boxes.end(); i++)
    {
      for (j = i + 1; j != boxes.end(); j++)
      {
        ...
        case LINE_VERTICAL:
          ...
          //i.e. (x12 - x21) / (x22 - x21)
          overlap = i->x < j->x ? (i->x + i->width - j->x) / (float)j->width : (j->x + j->width - i->x) / (float)i->width;
        ...
        case LINE_HORIZONTAL:
          ...
          //i.e. (y12 - y21) / (y22 - y21)
          overlap = i->y < j->y ? (i->y + i->height - j->y) / (float)j->height : (j->y + j->height - i->y) / (float)i->height;
...
//Sunny  /**** center point 190.4****/
void PointWorld2ImIPM(CvPoint2D32f &point, CvPoint2D32f &new_point,
                       IPMInfo *ipmInfo)
{
  //x-direction
  new_point.x = point.x*ipmInfo->xScale+ ipmInfo->xLimits[0]*ipmInfo->xScale*-1;
                       ...
                       LineType lineType,LineState *state,IPMInfo *ipmInfo)
{
  ...
  {
    //cout << "\n"<<((lines[i].startPoint.x-cp.x)/40);
    //cout << "\n"<<(((lines[i].startPoint.x-cp.x)/40)>0? 
                //(int)((lines[i].startPoint.x-cp.x)/40+1.0):(int)((lines[i].startPoint.x-cp.x)/40-1.0));
    lines[i].label = (((lines[i].endPoint.x-cp.x)/45)>0? 
                (int)((lines[i].endPoint.x-cp.x)/45+1.0):(int)((lines[i].endPoint.x-cp.x)/45-1.0));
    if(lines[i].label>MaxL)
      MaxL = lines[i].label;
...
void mcvGetRansacLines(const CvMat *im, vector<Line> &lines,
                       vector<float> &lineScores, LaneDetectorConf *lineConf,
                       LineType lineType,LineState *state)
{
  ...
  float pre_state_theta;
  int n =0;
  
  if(state->ipmSplines.size()>0){
    for (int i = 0; i < (int)state->ipmSplines.size(); i++,n++){
      pre_state_theta += state->ipmSplines[i].tangent;
    }
    pre_state_theta /= n;
  }
...
#warning "check this screening in ransacLines"
    ...
    { //#ifdef DEBUG_GET_STOP_LINES
      ...
      case LINE_HORIZONTAL:
        ...
        break;
      case LINE_VERTICAL:
        printf("Subimage Line in each boxes");
        sprintf(str, "#%d Subimage Line V #%d", i);
        break;
      }
      //convert image to rgb
  ...
  { //#ifdef DEBUG_GET_STOP_LINES
    ...
    { 
      if (fabs(lines[j].rtheta[1]-pre_state_theta) >0.1)
        ...
        mcvDrawLine(imClr, lines[j], CV_RGB(1, 0, 0), 1);   
      mcvDrawPoints(imClr, cvPoint2D32f(190.4,100), CV_RGB(1, 1, 0), 3);   
      SHOW_IMAGE(imClr, title, 10);
    }
    //draw spline
    //for (unsigned int j = 0; j < oldLines.size(); j++)
    //  mcvDrawLine(imClr2, oldLines[j], CV_RGB(1, 0, 0), 1);
    //SHOW_IMAGE(imClr2, "5. Input Lines", 10);
...
/** This function sets the matrix to a value except for the polygon mask window passed in
 *
 * \param inMat input matrix
 ...
 * \return computed e1 e2 e3 e4 for lane model propagation
 */
  cv::RNG rng((unsigned)time(NULL));
CvMat *mcvEvalBezierSplinediff(const Spline &spline, CvMat *y,
                                    LaneDetectorConf *stopLineConf)
{
  float Md[] = {0, 0, 0, 0,
              -3, 9, -9, 3,
  ...
  float y0 = CV_MAT_ELEM(*y, float, 0, 0);
  float y1 = CV_MAT_ELEM(*y, float, 0, 1);
  float y2 = CV_MAT_ELEM(*y, float, 0, 2);
  float y3 = CV_MAT_ELEM(*y, float, 0, 3);

  float sig1 = rng.uniform(y2, y3);
  float sig2 = rng.uniform(y1, y2);
  float sig3 = rng.uniform(y0, y1);
  float eta1 = rng.uniform(sig1, sig2);
  float eta2 = rng.uniform(sig2, sig3);
  ...
  //the return value //
  CvMat *e;
  e = cvCreateMat(1, 4, CV_32FC1);
  CvMat *t,*d,*dd,*ddd,*temp_ddd;
  float u=0.0;
  float diff = y3;
  t = cvCreateMat(1, 4, CV_32FC1);
  temp_ddd = cvCreateMat(4, 2, CV_32FC1);
  CvMat M,M1,M2,M3;
  M = cvMat(4, 4, CV_32FC1, Md);
  M1 = cvMat(4, 4, CV_32FC1, Mdd);
  M2 = cvMat(4, 4, CV_32FC1, Mddd);
  // d  dx//
  cvMatMul(&M, sp, temp_ddd);
  u = y3/diff ;
  CV_MAT_ELEM(*t, float, 0, 3) = 1;         //1
  CV_MAT_ELEM(*t, float, 0, 2) = u;         //u
  CV_MAT_ELEM(*t, float, 0, 1) = u * u;     //u^2
  CV_MAT_ELEM(*t, float, 0, 0) = u * u * u; //u^2
  d = cvCreateMat(1, 2, CV_32FC1);
  cvMatMul(t,temp_ddd,d);  //d =f'(y0)
  CV_MAT_ELEM(*e,float,0,0) = CV_MAT_ELEM(*d,float,0,0)/CV_MAT_ELEM(*d,float,0,1);
  cout << "\n%%" << sig1 << "%%"<< sig2-sig1<< "%%"<< sig3-sig2;
  //dd  e1//
  cvMatMul(&M1, sp,temp_ddd);
  u= sig1/diff;
  CV_MAT_ELEM(*t, float, 0, 3) = 1;         //1
  CV_MAT_ELEM(*t, float, 0, 2) = u;         //u
  CV_MAT_ELEM(*t, float, 0, 1) = u * u;     //u^2
  CV_MAT_ELEM(*t, float, 0, 0) = u * u * u; //u^2
  //dd = cvCreateMat(1, 2, CV_32FC1);
  cvMatMul(t,temp_ddd,d);  //dd =f''(sig1)
  CV_MAT_ELEM(*e,float,0,1) = CV_MAT_ELEM(*d,float,0,0)/CV_MAT_ELEM(*d,float,0,1);

  //ddd  e2//
  cvMatMul(&M2, sp,temp_ddd);
  u= eta1/diff;
  CV_MAT_ELEM(*t, float, 0, 3) = 1;         //1
  CV_MAT_ELEM(*t, float, 0, 2) = u;         //u
  CV_MAT_ELEM(*t, float, 0, 1) = u * u;     //u^2
  CV_MAT_ELEM(*t, float, 0, 0) = u * u * u; //u^2
  //ddd = cvCreateMat(1, 2, CV_32FC1);
  cvMatMul(t,temp_ddd,d);  //ddd =f'''(eta1)
  CV_MAT_ELEM(*e,float,0,2) = CV_MAT_ELEM(*d,float,0,0)/CV_MAT_ELEM(*d,float,0,1);
  //ddd  e3//
  cvMatMul(&M2, sp,temp_ddd);
  u= eta2/diff;
  CV_MAT_ELEM(*t, float, 0, 3) = 1;         //1
  ...
  CV_MAT_ELEM(*t, float, 0, 0) = u * u * u; //u^2
  //ddd = cvCreateMat(1, 2, CV_32FC1);
  cvMatMul(t,temp_ddd,d);  //ddd =f'''(eta2)
  CV_MAT_ELEM(*e,float,0,3) = CV_MAT_ELEM(*d,float,0,0)/CV_MAT_ELEM(*d,float,0,1);
  cout << "\n$$$$$$" <<  CV_MAT_ELEM(*e,float,0,3) <<"\t"<< CV_MAT_ELEM(*e,float,0,2)<<
  "\t"<<CV_MAT_ELEM(*e,float,0,1)<<"\t"<<  CV_MAT_ELEM(*e,float,0,0);
  return e;
...
void mcvGetRansacSplines(const CvMat *im, vector<Line> &lines,
                         ...
                         vector<float> &splineScores,vector<Spline> &bestSplines,
                         vector<float> &bestSplineScores, LineState *state, IPMInfo *ipmInfo)
{
  //check if to binarize image
  CvMat *image = cvCloneMat(im);
...
cin.get();
  ...
  //Turned off adding boxes from previous frame -Sunny
  //boxes.insert(boxes.end(), state->ipmBoxes.begin(),
  //state->ipmBoxes.end());
  //mcvGroupBoundingBoxes(boxes, lineType, overlapThreshold);
  ...
  for (int i = 0; i < (int)boxes.size(); i++) //lines
  {
    ...
    /*mcvFitRansacLine(subimage, lineConf->ransacLineNumSamples,
                     ...
                     lineConf->ransacLineNumGoodFit,
                     lineConf->getEndPoints, lineType,
                     &tlines[i], &tlines[i].rtheta[1], &tlines[i].score);*/
    //get the RANSAC spline in this part
    //int numSamples = 5, numIterations = 10, numGoodFit = 15;
    //float threshold = 0.5;
    Spline spline;
    float splineScore;
    vector<Spline> iterSplines;
    vector<float> iterScores; //Sunny
    ...
    /* mcvFitRansacSpline(subimage, lineConf->ransacSplineNumSamples,
                       ...
                       &state->ipmSplines);*/
    mcvFitRansacSpline_(subimage, lineConf->ransacSplineNumSamples,
                        lineConf->ransacSplineNumIterations,
                        lineConf->ransacSplineThreshold,
                        lineConf->ransacSplineScoreThreshold,
                        lineConf->ransacSplineNumGoodFit,
    ...
    if (spline.degree > 0)
    {
      bestSplines.push_back(spline);
      newSplines.insert(newSplines.end(), iterSplines.begin(), iterSplines.end());
      bestSplineScores.push_back(splineScore);
      newSplineScores.insert(newSplineScores.end(), iterScores.begin(), iterScores.end());
    }
    ...
    { //#ifdef DEBUG_GET_STOP_LINES
      ...
      {
      case LINE_HORIZONTAL:
        sprintf(title, " #%d Subimage Spline H", i);
        break;
      case LINE_VERTICAL:
        sprintf(title, "#%d Subimage Spline V ", i);
        printf("\n6. subimage Spline");
        break;
      }
      //convert image to rgb
      ...
      //put text
      sprintf(str, " 6.subimage score=%.2f", splineScore);
...
void mcvFitRansacSpline(const CvMat *image, int numSamples, int numIterations,
                        float threshold, float scoreThreshold, int numGoodFit,
                        int splineDegree, float h, Spline *bestspline,
                        float *bestsplineScore, int splineScoreJitter,
...
#warning "check adding half to points"
  ...
  for (int i = 0; i < numIterations; i++)
  {
    ...
    else
    {
      ...
      curSplines.push_back(curSpline);
    ...
    if (0)
    { //DEBUG_LINES) {//#ifdef DEBUG_GET_STOP_LINES
  ...
  //return
  if (bestspline)
    *bestspline = bestSpline;
  if (bestsplineScore)
    *bestsplineScore = bestScore;
...
//Sunny
/** This functions implements RANSAC algorithm for spline fitting
 *  given an image and keep all the iteration splines
 *
 *
 * \param image input image
 * \param numSamples number of samples to take every iteration
 ...
 *   pass NULL to ignore this input
 */
void mcvFitRansacSpline_(const CvMat *image, int numSamples, int numIterations,
                         float threshold, float scoreThreshold, int numGoodFit,
                         int splineDegree, float h, Spline *bestspline,
                         float *bestsplineScore, int splineScoreJitter,
                         float splineScoreLengthRatio,
                         float splineScoreAngleRatio, float splineScoreStep,
                         vector<Spline> &splines, vector<float> &scores, vector<Spline> *prevSplines)
{
  //get the points with non-zero pixels
  CvMat *points = mcvGetNonZeroPoints(image, true);
  if (points == 0 || points->cols < numSamples)
...
#warning "check adding half to points"
  ...
  for (int i = 0; i < numIterations; i++)
  {
    ...
    //printf("Score=%.2f & scoreThreshold=%.2f\n", score, scoreThreshold);
    if (score >bestScore &&score >= scoreThreshold && first ==false)// || abs(prevSpline->cur) <5 ||abs(prevSpline->cur) >100) )//Sunny06.06
    {
      //put it
      bestScore = score;
                      ...
                      CameraInfo &cameraInfo, CvSize imSize)
{
  ...
  if (lines.size() != 0)
  {
    ...
    //convert to mat and get in image coordinatesa
                        ...
                        CameraInfo &cameraInfo, CvSize imSize)
{
  ...
  for (int i = 0; i < (int)splines.size(); i++)
  {
    ...
    //mcvPointImIPM2World(CvMat *mat,  IPMInfo *ipmInfo);
                       ...
                       float angleThreshold)
{
  ...
  for (int i = 0; i < inPoints->height; i++)
  {
    ...
    { //#ifdef DEBUG_GET_STOP_LINES
      ...
      //output points
      cvCircle(imageClr, cvPoint((int)CV_MAT_ELEM(*outPoints, float, i, 0), (int)CV_MAT_ELEM(*outPoints, float, i, 1)),
               1, CV_RGB(0, 1, 0), -1);
      //input points
      cvCircle(imageClr, cvPoint((int)(line.startPoint.x + line.endPoint.x) / 2, (int)(line.startPoint.y + line.endPoint.y) / 2),
               1, CV_RGB(1, 0, 0), -1);
      //show image
      SHOW_IMAGE(imageClr, str, 10);
    } //#endif
                    ...
                    const CvPoint2D32f &prevPoint, float angleThreshold)
{
  ...
  float angle = peakTangent.x * tangent.x + peakTangent.y * tangent.y;
  if (DEBUG_LINES)
    fprintf(stderr, "angle=%f\n", angle);
  //return
  return (fabs(angle) >= angleThreshold && peakTangent.x*tangent.x>=0 ) ? true : false;
}
 ...
 *
 */
CvMat *mcvExtendPoints_lsTangent(const CvMat *im, const CvMat *inPoints,
                       ...
                       CvRect bbox,Spline spline,  bool smoothPeaks)
{
  //size of inPoints must be at least 3
  ...
  /*least square direction*/
  //float curLineRTheta[2],curLineAbc[3];
  //        CvMat *cpointst = cvCreateMat(2, inPoints->height, CV_32FC1);
  //      cvTranspose(inPoints, cpointst);
  //      mcvFitRobustLine(cpointst, curLineRTheta, curLineAbc);
  char str[256];
  CvMat *imageClr;
  if (DEBUG_LINES)
  { //#ifdef DEBUG_GET_STOP_LINES
    ...
    for (int i = 0; i < inPoints->height; i++)
      //input points
      cvCircle(imageClr, cvPoint((int)(CV_MAT_ELEM(*inPoints, float, i, 0)), (int)(CV_MAT_ELEM(*inPoints, float, i, 1))),
               1, CV_RGB(0, 1, 1), -1);
    /*Draw ls direction*/ // need to delete after 
    //  cvLine(imageClr, cvPoint((int)((-curLineAbc[1]*10-curLineAbc[2])/curLineAbc[0]),10 ),
    //          cvPoint((int)((-curLineAbc[1]*200-curLineAbc[2])/curLineAbc[0]),200 ),
    //           CV_RGB(1, 0, 1));
    //show image
    SHOW_IMAGE(imageClr, str, 10);
  } //#endif
   ...
   //cvReleaseMat(&cpointst);
  //tangent and normal
  CvPoint2D32f tangent, curPoint, peak, nextPoint, meanDir,t;
                     ...
                     bool smoothPeaks)
{
  ...
  if (maximaLoc.size() > 0)
  {
    ...
    for (unsigned int i = 0; i < maximaLoc.size(); ++i)
    {
      ...
      double val3 = CV_MAT_ELEM(*pixStep, float, 0, MIN(maximaLoc[i] + 1, pixStep->width - 1));
                        ...
                        int jitterVal, float lengthRatio, float angleRatio)
{
  ...
  if (0)
    fprintf(stderr, "raw score=%.2f, angle=%.2f, length=%.2f, final=%.2f\n",
            score, angle, length, score * (1 + (angleRatio * angle + lengthRatio * length) / 2));
...
CvPoint2D32f mcvGetPointsLsVector(const CvMat *points, bool forward)
{
  CvPoint2D32f mean, v;
 ...
 * \param boxes a vector of output bounding boxes
 */
void mcvGetSplinesBoundingBoxesLabel(vector<Spline> &splines, LineType type,
                                     CvSize size, vector<CvRect> &boxes)
{
  //copy lines to boxes
  int start, end;
  vector<CvRect> new_boxes;
  //clear
  //boxes.clear();
  switch (type)
  {
  ...
  case LINE_HORIZONTAL:
    for (unsigned int i = 0; i < splines.size(); ++i)
    {
      ...
      start = (int)fmin(splines[i].points[0].y,
                      ...
                      splines[i].points[splines[i].degree].y);
      new_boxes.push_back(cvRect(0, start, size.width - 1, end - start + 1));

      splines[i].boxlabel = start;
      splines[i].label = i;
    }
    break;
  }
...
//Sunny
/** \brief This function label spines
 *
 * \param splines vector of splines
 * \param type the type of lines (LINE_HORIZONTAL or LINE_VERTICAL)
 * \param size the size of image containing the lines
 * \param splines vector of state
 */
/*void mcvGetSplinesLabel(vector<Spline> &splines, LineType type,
                                CvSize size, vector<Spline> &state_splines)
{
  cout << "\nDebug  mcvGet Splines Label\n";
  if(state_splines.size()==0 )
  {
    cout << "Initial Splines Label\n";
    ...
    cout << "start labeling\n";
    CvMat *Vector[1];
    CvMat* Vector1 = cvCreateMat(4,2,CV_32FC1);
    CvMat *Vector2 = cvCreateMat(1,2,CV_32FC1);
    CvMat* CovarMatrix =cvCreateMat(2,2,CV_32FC1);
    CvMat* InvertCovarMatrix=cvCreateMat(2,2,CV_32FC1);
    CvMat* AvgVector=cvCreateMat(1,2,CV_32FC1);
    float sum_dis,min_dis;
    for(int j=0;j<splines.size();j++){ //new detected
      min_dis=30;
      for(int i=0;i<state_splines.size();i++){ //origin state
        Vector1 = mcvCvPoint2D32f2Mat(state_splines[i].points);
        Vector[0]=Vector1;
        cvCalcCovarMatrix((const CvArr **)Vector,4,CovarMatrix,AvgVector,CV_COVAR_SCALE+CV_COVAR_NORMAL+CV_COVAR_ROWS);
        cvInvert(CovarMatrix,InvertCovarMatrix,CV_SVD_SYM);
      
        sum_dis =0;
        for(int k=0;k<4;k++){
          float Coordinates2[2]={splines[j].points[k].x,splines[j].points[k].y};
          cvSetData(Vector2,Coordinates2,Vector2->step);
          sum_dis+= cvMahalanobis(Vector2,AvgVector,InvertCovarMatrix);

        }
        if(sum_dis< min_dis){
          splines[j].label= state_splines[i].label;
...
/*void mcvGetSplinesLabel(vector<Spline> &bestSplines, vector<float> &bestSplineScores,vector<Spline> &splines, 
                        vector<float> &splineScores, LineType type, CvSize size, vector<Spline> &state_splines)
{
  vector<Spline> tempSplines;
  vector<float> tempSplineScores;
  tempSplines = splines;
  tempSplineScores = splineScores;
  int mark=0;
  //cout << "\nDebug  mcvGetSplines Label\n";
  if (state_splines.size() == 0)
  {
    //group the splines
    mcvGroupSplines(bestSplines, bestSplineScores);
    cout << "\nInitial Splines Label" << bestSplines.size();
    CvMat *Vector[1];
    ...
    {
      CvMat *points = mcvEvalBezierSpline(splines[j], .01);
      min_dis = 200;
      for (int i = 0; i < bestSplines.size(); i++)
      { //temp state
        CvMat *temp_points = mcvEvalBezierSpline(bestSplines[i], .01);
        Vector[0] = temp_points;
        cvCalcCovarMatrix((const CvArr **)Vector, temp_points->height, CovarMatrix, AvgVector, CV_COVAR_SCALE + CV_COVAR_NORMAL + CV_COVAR_ROWS);
        cvInvert(CovarMatrix, InvertCovarMatrix, CV_SVD_SYM);
        sum_dis = 0;
        for (int k = 0; k < points->height; k++)
        {
          float Coordinates2[2] = {CV_MAT_ELEM(*points, float, k, 0), CV_MAT_ELEM(*points, float, k, 1)};
          cvSetData(Vector2, Coordinates2, Vector2->step);
          sum_dis += cvMahalanobis(Vector2, AvgVector, InvertCovarMatrix);
        }
        //printf("\n%f\n", sum_dis);
    ...
    vector<Spline>::iterator j;int m;
    for ( m=0,j=splines.begin(); j !=splines.end();m++, j++)
    { 
      if(splines[m].label==-1)
        splines.erase(j);
    }   
  ...
  else
  {
    ...
    //cout  <<state_splines.size() <<"start labeling\n";
    CvMat *Vector[1];
    CvMat *Vector1 = cvCreateMat(4, 2, CV_32FC1);
    CvMat *Vector2 = cvCreateMat(1, 2, CV_32FC1);
    CvMat *CovarMatrix = cvCreateMat(2, 2, CV_32FC1);
    ...
    { //new detected
      CvMat *points = mcvEvalBezierSpline(splines[j], .01);
      min_dis = 200;
      for (int i = 0; i < state_splines.size(); i++)
      { //origin state
        CvMat *state_points = mcvEvalBezierSpline(state_splines[i], .01);
        Vector[0] = state_points;
        cvCalcCovarMatrix((const CvArr **)Vector, state_points->height, CovarMatrix, AvgVector, CV_COVAR_SCALE + CV_COVAR_NORMAL + CV_COVAR_ROWS);
        cvInvert(CovarMatrix, InvertCovarMatrix, CV_SVD_SYM);
        ...
        for (int k = 0; k < points->height; k++)
        {
          float Coordinates2[2] = {CV_MAT_ELEM(*points, float, k, 0), CV_MAT_ELEM(*points, float, k, 1)};
          cvSetData(Vector2, Coordinates2, Vector2->step);
          sum_dis += (k+5)/100*cvMahalanobis(Vector2, AvgVector, InvertCovarMatrix);
        }
                  //printf("%f\n", sum_dis);
        if (sum_dis < min_dis)
        {
...
/* Sunny */
/** \brief This function takes a bunch of lines, and check which
 * 2 lines might be a lane where the vehicle is in.
 *
 * \param lines vector of lines
 * \param scores vector of line scores
 */
...
void mcvTwoLanes(vector<Line> &lines, vector<float> &scores, CvSize size, float wMu)
{
  ...
  for (i = 0, ir = rs.begin(); ir != rs.end(); ir++, i++)
  {
    ...
    //get that score
    score = fabs((*ir +  *(ir + 1)) / 2 - size.width / 2 )+ 0.5 * (fabs(*ir - *(ir + 1)) - wMu)+10*fabs(lines[i].rtheta[1]-lines[i+1].rtheta[1]);
